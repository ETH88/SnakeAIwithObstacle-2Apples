...
#from counter import *
global applecount, apple2count ##
applecount=0 ##
apple2count =0 ##


class Vision(object):
    __slots__ = ('dist_to_wall', 'dist_to_apple', 'dist_to_apple2', 'dist_to_obstacle', 'dist_to_self') ##
    #__slots__ = ('dist_to_wall', 'dist_to_apple', 'dist_to_self')
    def __init__(self,
                 dist_to_wall: Union[float, int],
                 dist_to_apple: Union[float, int],
                 dist_to_apple2: Union[float, int],
                 dist_to_obstacle: Union[float, int], ##
                 dist_to_self: Union[float, int]
                 ):
        self.dist_to_wall = float(dist_to_wall)
        self.dist_to_apple = float(dist_to_apple)
        self.dist_to_apple2 = float(dist_to_apple2)
        self.dist_to_obstacle = float(dist_to_obstacle) ##
        self.dist_to_self = float(dist_to_self)

class DrawableVision(object):
    __slots__ = ('wall_location', 'apple_location', 'apple2_location', 'obstacle_location', 'self_location')##
    def __init__(self,
                wall_location: Point,
                apple_location: Optional[Point] = None,
                apple2_location: Optional[Point] = None,
                obstacle_location: Optional[Point] = None,##
                self_location: Optional[Point] = None,
                ):
        self.wall_location = wall_location
        self.apple_location = apple_location
        self.apple2_location = apple2_location
...
class Snake(Individual):
    def __init__(self, board_size: Tuple[int, int],
        ...
        #num_inputs = len(self._vision_type) * 3 + 4 + 4 #@TODO: Add one-hot back in #ORIGINAL
        num_inputs = len(self._vision_type) * 5 + 4 + 4 #@TODO: Add one-hot back in   ##
        ##now 4 visions (obstacle)
        ...
        if apple2_seed is None: ##
            apple2_seed = np.random.randint(-1000000000, 1000000000)
        self.apple2_seed = apple2_seed  # Only needed for saving/loading replay
        self.rand_apple2 = random.Random(self.apple2_seed)
        
        if obstacle_seed is None:  ##
            obstacle_seed = np.random.randint(-1000000000, 1000000000)
        self.obstacle_seed = obstacle_seed  # Only needed for saving/loading replay
        self.rand_obstacle = random.Random(self.obstacle_seed)

        self.apple_location = None
        self.apple2_location = None ##
        ...
        self.generate_apple() #CANNOT GENERATE APPLE ON  TOP OF OBSTACLE
    ...
    def look_in_direction(self, slope: Slope) -> Tuple[Vision, DrawableVision]:
        ...
        dist_to_apple2 = np.inf
        dist_to_obstacle = np.inf ##
        dist_to_self = np.inf

        wall_location = None
        apple_location = None
        apple2_location = None
        ...
        while self._within_wall(position):
            if not body_found and self._is_body_location(position):
                ...
                body_found = True
            if not food2_found and self._is_apple2_location(position): ##
                dist_to_apple2 = total_distance
                apple2_location = position.copy()
                food2_found = True
            if not food1_found and self._is_apple_location(position):
                dist_to_apple = total_distance
                apple_location = position.copy()
                food1_found = True
                '''
            if not food_found and self._is_apple2_location(position): ##
                dist_to_apple2 = total_distance
                apple2_location = position.copy()
                food_found = True
                '''
            if not obstacle_found and self._is_obstacle_location(position): ## DONT KNOW WHAT  OBSTACLE_FOUND DOES???
                dist_to_obstacle = total_distance
                obstacle_location = position.copy()
                obstacle_found = True
        ...
        if self.apple_and_self_vision == 'binary':
            ...
            dist_to_apple2 = 1.0 if dist_to_apple2 != np.inf else 0.0 ##
            dist_to_obstacle = 1.0 if dist_to_obstacle != np.inf else 0.0 ##
            dist_to_self = 1.0 if dist_to_self != np.inf else 0.0

        elif self.apple_and_self_vision == 'distance':
            dist_to_apple = 1.0 / dist_to_apple
            dist_to_apple2 = 1.0 / dist_to_apple2 ##
            dist_to_obstacle = 1.0 / dist_to_obstacle ##
            dist_to_self = 1.0 / dist_to_self

        vision = Vision(dist_to_wall, dist_to_apple, dist_to_apple2, dist_to_obstacle, dist_to_self) ##
        drawable_vision = DrawableVision(wall_location, apple_location, apple2_location, obstacle_location, self_location) ##
    ...
    def _vision_as_input_array(self) -> None:
        ...
        for va_index, v_index in zip(range(0, len(self._vision) * 5, 5), range(len(self._vision))): ##adding obstacle vision
            ...
            self.vision_as_array[va_index + 2, 0] = vision.dist_to_apple2 ##
            self.vision_as_array[va_index + 3, 0] = vision.dist_to_obstacle ##
            self.vision_as_array[va_index + 4, 0] = vision.dist_to_self
            #self.vision_as_array[va_index + 2, 0] = vision.dist_to_self

        i = len(self._vision) * 5 ##
    ...
    def generate_apple(self) -> None:
        ...
        if possibilities: ##
            while(True):
                loc = self.rand_apple.choice(possibilities)
                self.apple_location = Point(loc[0], loc[1])
                if (self.apple_location != self.obstacle_location and self.apple_location != self.apple2_location):  ##MAY CAUSE ERROR!
                #if (self.obstacle_location != self.apple_location and self.obstacle_location != self.apple2_location): 
                    #MAKING SURE OBSTACLE DOESNT SPAWN ON TOP OF APPLES
                    break
        else:
            # I guess you win?
            print('you won!')
    ...
    def generate_apple2(self) -> None: ##
        ...
        if possibilities: ##
            while(True):
                loc = self.rand_apple2.choice(possibilities)
                self.apple2_location = Point(loc[0], loc[1])
                if (self.apple2_location != self.obstacle_location and self.apple2_location != self.apple_location): 
                #if (self.obstacle_location != self.apple_location and self.obstacle_location != self.apple2_location): 
                    #MAKING SURE OBSTACLE DOESNT SPAWN ON TOP OF APPLES
                    break
        else:
    ...
    def generate_obstacle(self) -> None: ##
        ...
        possibilities = [divmod(i, height) for i in range(width * height) if divmod(i, height) not in self._body_locations]
        if possibilities:
            loc = self.rand_obstacle.choice(possibilities)
            self.obstacle_location = Point(loc[0], loc[1])
        else:
            # I guess you win?
            print('you won!')
            pass
    ...
    def move(self) -> bool:
        ...
        if self._is_valid(next_pos):
            ...
            # Eat the apple
                ...
                self.generate_apple()
            elif next_pos == self.apple2_location: ##
                self.score += 1##
                apple2count +=1
                print('apple2:', apple2count)
                self._frames_since_last_apple = 0
                # Move head
                self.snake_array.appendleft(next_pos)
                self._body_locations.update({next_pos})
                # Don't remove tail since the snake grew
                self.generate_apple2()
            #elif next_pos == self.obstacle_location: ## learns not to hit wall and by not decreasing score, so should be able to learn not to hit obstacle?
                #self.score -= 1 ## ENOUGH? - play with
            # Normal movement
            else:
                # Move head
                self.snake_array.appendleft(next_pos)
    ...
    def _is_apple2_location(self, position: Point) -> bool:
        return position == self.apple2_location
    
    def _is_obstacle_location(self, position: Point) -> bool:
        return position == self.obstacle_location
    ...
    def _is_valid(self, position: Point) -> bool:
        ...
        """
        if position == self.obstacle_location: ##
            return False
...
def save_snake(population_folder: str, individual_name: str, snake: Snake, settings: Dict[str, Any]) -> None:
    ...
    constructor['apple2_seed'] = snake.apple2_seed ##
    constructor['obstacle_seed'] = snake.obstacle_seed ##
...
def load_snake(population_folder: str, individual_name: str, settings: Optional[Union[Dict[str, Any], str]] = None) -> Snake:
    ...
    snake = Snake(settings['board_size'], chromosome=params, 
                  ...
                  apple2_seed=constructor_params['apple2_seed'], ##
                  obstacle_seed=constructor_params['obstacle_seed'], ##
                  ...
                  apple2_and_self_vision=settings['apple2_and_self_vision'])
                  #)
    return snake