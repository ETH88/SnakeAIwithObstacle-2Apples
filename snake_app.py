...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, settings, show=True, fps=1000000000):
        super().__init__()
    ...
    def update(self) -> None:
        #global applecount,snake.apple2count
        ...
        # Current individual is dead         
            ...
            if (self.current_generation > 0 and self._current_individual == self._next_gen_size) or\
                ...
                #print('----Apple 1 count:', snake.applecount)
                #print('----Apple 2 count:', snake.apple2count)
                save_stats(self.population, '/Users/ethanhsiao/Desktop/Trial3', 'fname')  ##
                #x=load_stats('/Users/ethanhsiao/Desktop/Comsci/Python/CS\ Capstone/Snake/SnakeAI-master_original/Population', True) ##
                #print(x)
    ...
    def next_generation(self):
        ...
        if self.settings['selection_type'].lower() == 'plus':
            ...
            for individual in self.population.individuals:
                ...
                apple2_seed = individual.apple2_seed ##
...
class GeneticAlgoWidget(QtWidgets.QWidget):
    def __init__(self, parent, settings):
        ...
        network_architecture = '[{}, {}, 4]'.format(settings['vision_type'] * 5 + 4 + 4,
...
class SnakeWidget(QtWidgets.QWidget):
    ...
    def draw_snake(self, painter: QtGui.QPainter) -> None:
        ...
        def _draw_line_to_apple2(painter: QtGui.QPainter, start_x: int, start_y: int, drawable_vision: DrawableVision) -> Tuple[int, int]: ##
            painter.setPen(QtGui.QPen(Qt.blue))
            end_x = drawable_vision.apple2_location.x * SQUARE_SIZE[0] + SQUARE_SIZE[0]/2
            end_y = drawable_vision.apple2_location.y * SQUARE_SIZE[1] + SQUARE_SIZE[1]/2
            painter.drawLine(start_x, start_y, end_x, end_y)
            return end_x, end_y
                ...
                for drawable_vision in self.snake._drawable_vision:
                    ...
                    if drawable_vision.apple_location and drawable_vision.self_location:
                        ...
                        if apple_dist <= self_dist:
                            ...
                            start_x, start_y = _draw_line_to_self(painter, start_x, start_y, drawable_vision)
                            start_x, start_y = _draw_line_to_apple(painter, start_x, start_y, drawable_vision)
                    if drawable_vision.apple2_location and drawable_vision.self_location: ##
                        apple2_dist = self._calc_distance(start.x, drawable_vision.apple2_location.x, start.y, drawable_vision.apple2_location.y)
                        self_dist = self._calc_distance(start.x, drawable_vision.self_location.x, start.y, drawable_vision.self_location.y)
                        if apple2_dist <= self_dist:
                            start_x, start_y = _draw_line_to_apple2(painter, start_x, start_y, drawable_vision)
                            start_x, start_y = _draw_line_to_self(painter, start_x, start_y, drawable_vision)
                        else:
                            start_x, start_y = _draw_line_to_self(painter, start_x, start_y, drawable_vision)
                            start_x, start_y = _draw_line_to_apple2(painter, start_x, start_y, drawable_vision)
                        ...
                        start_x, start_y = _draw_line_to_apple(painter, start_x, start_y, drawable_vision)
                    elif drawable_vision.apple2_location:
                        start_x, start_y = _draw_line_to_apple2(painter,start_x,start_y, drawable_vision)
    ...
    def draw_apple2(self, painter: QtGui.QPainter) -> None: ##
        apple2_location = self.snake.apple2_location
        if apple2_location:
            painter.setRenderHints(QtGui.QPainter.HighQualityAntialiasing)
            painter.setPen(QtGui.QPen(Qt.black))
            painter.setBrush(QtGui.QBrush(Qt.blue))

            painter.drawRect(apple2_location.x * SQUARE_SIZE[0],
                             apple2_location.y * SQUARE_SIZE[1],
                             SQUARE_SIZE[0],
                             SQUARE_SIZE[1])
            
    def draw_obstacle(self, painter: QtGui.QPainter) -> None: ##
        obstacle_location = self.snake.obstacle_location
        if obstacle_location:
            painter.setRenderHints(QtGui.QPainter.HighQualityAntialiasing)
            painter.setPen(QtGui.QPen(Qt.black))
            painter.setBrush(QtGui.QBrush(Qt.yellow))

            painter.drawRect(obstacle_location.x * SQUARE_SIZE[0],
                             obstacle_location.y * SQUARE_SIZE[1],
                             SQUARE_SIZE[0],
                             SQUARE_SIZE[1])
